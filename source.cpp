...
holder* insert_New(holder* hold, char x) {
	if (find(hold, x) == false) {
		hold = insert(hold, x);
	}
	else {
		holder* tmp = hold;
...
decimalnum* insertLast(decimalnum* h, int item)
{
	if (h == NULL)
	{
...
int main()
{
	...
	while (tf == NULL) {
		printf("\n\t\t\t\t\tHello\n To start conversion please type the file name here :- ");
	...
	do {
		if (choice == 1) {
			while (ftell(tf) < ss.st_size) {
				fscanf(tf, "%c", &current);
				Letters_Holder = insert_New(Letters_Holder, current);
			}
			while (Letters_Holder != NULL) {
				Frequency_Table = Insert_Ascending(Frequency_Table, Letters_Holder->freq, Letters_Holder->charecter);
				Letters_Holder = Letters_Holder->next;
			}
			//---------------Extracting File Name--------------//
			char fileNamecod[1000] = "";
			char filenamecom[1000] = "";
			char filenamedecom[1000] = "";
			...
			*file = '\0';
			...
			strcpy(filenamedecom, fileNamecod);
			strcat(filenamedecom, " decom.txt");
			...
			stat(fileNamecod, &gg);
			cod = (char*)malloc(sizeof(char) * gg.st_size + 1);
			...
			compressed = (unsigned char*)malloc(sizeof(unsigned char) * c);
			...
			FILE* e;
			e = fopen(filenamecom, "ab");
			fprintf(e, "%d", cnt);
			fwrite(compressed, sizeof(unsigned char), c, e);
			fclose(e);
			printf("\nFile was compressed successfully as %s\n", filenamecom);
		}
		if (choice == 2) {
			char fileNamecod[1000] = "";
			char filenamecom[1000] = "";
			...
			*file = '\0';
			...
			int NUMM = 0;
			FILE* X;
			X = fopen(filenamecom, "rb");
			struct stat bb;
			stat(filenamecom, &bb);
			int* num;
			unsigned char aaa;
			num = (int*)malloc(sizeof(int) * bb.st_size);
			fscanf(X, "%d", &extraBits);
			if (extraBits == 0) {
				for (int i = 1; i < bb.st_size - 1; i++) {
					fscanf(X, "%c", &aaa);
					num[i - 1] = aaa;
					NUMM++;
				}
			...
			fclose(X);
			int start_counter = 0;
			int num_counter = NUMM;
			char* binary = (char*)malloc(sizeof(char) * (8 * num_counter - extraBits));
			binary[8 * num_counter - extraBits] = 0;
			for (int i = 0; i < num_counter; i++)
			{
				for (int j = 0; j < 8; j++)
					binary[(i * 8) + j] = ((num[i] >> (7 - j)) & 1) + '0';
				if (i == num_counter - 1) {
					for (int j = 0; j < 8 - extraBits; j++) {
						binary[(i * 8) + j] = binary[(i * 8) + j + extraBits];
					}
					binary[(i * 8) + (8 - extraBits)] = 0;
				}
			}
			char* z;
			z = (char*)malloc(sizeof(char) * strlen(binary));
			strcpy(z, "");
			decode(binary, input, z);
			FILE* WW;
			WW = fopen(filenamedecom, "w");
			if (WW == NULL)
			{
				printf("Error");
				return 0;;
			...
			fprintf(WW, "%s", z);
			fclose(WW);
			printf("\nThe file has been extracted successfully as %s.\n", filenamedecom);
		}
		if (choice == 3)
		{
